...
globals [
  total-ticks
  time-limit
  global-altitude  ; Variable global para almacenar la altura
  target-time      ; Variable global para el tiempo objetivo
  move-time
  who-am-i
  velocidad
]
...
to setup
  ...
  [ set size 2         ;; easier to see
    ...
    set pen-mode "down" ;; trayectoria de la hormiga
  ]
  set global-altitude 0  ; Inicializar la altura global en 0
  set target-time 10;    ; Establece el tiempo objetivo en 10 ticks
  set move-time 0
  set who-am-i 0
  set total-ticks 100
  set velocidad 1
...
;;CON ESTO CONTROLAS DONDE COLOCAR COMIDA
to setup-food  ;; patch procedure
  ...
  if (distancexy (0.0 * max-pxcor) (-0.5 * max-pycor)) < 2
  [ set food-source-number 5 ]
  ;; setup food source two on the lower-left
  if (distancexy (0.0 * max-pxcor) (0.5 * max-pycor)) < 2
  [ set food-source-number 6 ]
  ;; setup food source three on the upper-left
  if (distancexy (0.5 * max-pxcor) (0.0 * max-pycor)) < 2
  [ set food-source-number 7 ]
  if (distancexy (-0.5 * max-pxcor) (0.0 * max-pycor)) < 2
  [ set food-source-number 8 ]
  ;; set "food" at sources to either 1 or 2, randomly
  ;; TODO
  ;; asigna aleatoriamente un valor de 1 o 2 a la cantidad de comida en cada fuente.
  if food-source-number > 0
  [ set food one-of [1 100] ]
end

to recolor-patch  ;; patch procedure
  ;; verifica si el parche es un nido o no
  ifelse nest? ;; Si nest? es verdadero, lo que significa que el parche es un nido,
  [ set pcolor violet ] ;; se establece el color del parche en violeta.
  ;; Si el parche NO es un nido
  [ ifelse food > 0 ;; Verificamos si hay comida o no
    [ if food-source-number = 1 [ set pcolor pink ]
      if food-source-number = 2 [ set pcolor cyan  ]
      if food-source-number = 3 [ set pcolor blue ]
      if food-source-number = 4 [ set pcolor green ]
      if food-source-number = 5 [ set pcolor orange ]
      if food-source-number = 6 [ set pcolor sky  ]
      if food-source-number = 7 [ set pcolor red ]
      if food-source-number = 8 [ set pcolor gray ] ]
...
to go
  ask turtles [
    ...
    ifelse total-ticks > 0 [
        look-for-food ; Si aún tiene tiempo, busca comida
        set total-ticks total-ticks - 1
       ; Decrementa el contador de tiempo
      ] [
        return-to-nest
      ; Si se acabó el tiempo, vuelve al nido
      ]
    ...
    set move-time move-time - 1
      ...
      if who-am-i = 1[
      set global-altitude 200
      ]
      if who-am-i = 2[
      set global-altitude 0
      ]
...
to return-to-nest
  ifelse nest? [
    ...
    ;if (distancexy 0 0) < 5 and global-altitude > 200
    ;[set global-altitude global-altitude - 400
    ;plotxy ticks global-altitude
    ;set global-altitude global-altitude + 400
    ;plotxy ticks global-altitude]
  ]
...
to look-for-food
  ...
  [
    ...
    set food food - 0.25
    rt 180
    ;set global-altitude global-altitude - 200
    ;plotxy ticks global-altitude
    ;set global-altitude global-altitude + 200
    ;plotxy ticks global-altitude
    ;;
